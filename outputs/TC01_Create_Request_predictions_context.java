...
public class TC05_Admin_Menu_User_Management
{
    ...
    void Admin_Menu_User_Management()
    {
        IocBuilder.execute(Duration.ofMinutes(10), EResultData.ADMIN, "842", tc ->
        {
            ...
            //Step 5
            String searchCriteria = "TestUser";
            tc.edit.sendKeys(EEdit.SEARCH, searchCriteria);
            WaitFor.condition(() -> tc.table.getItemsFromColumn(ETable.USER_TABLE, EColumn.USERNAME)
                    .stream().anyMatch(name -> name.contains(searchCriteria)));
            boolean isSearchCorrect = tc.table.getItemsFromColumn(ETable.USER_TABLE, EColumn.USERNAME)
                    .stream().allMatch(name -> name.contains(searchCriteria));
            tc.addStepInfo("Only users matching search conditions are displayed", true, isSearchCorrect,
                    new ComparerOptions().takeScreenShotPlatform());

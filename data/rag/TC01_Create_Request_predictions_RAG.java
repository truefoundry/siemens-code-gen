...
public class TC05_Landing_Page_Content_Check
{
    ...
    void Landing_Page_Content_Check()
    {
        IocBuilder.execute(Duration.ofMinutes(10), EResultData.ADMIN, "842", tc ->
        {
            ...
            //Step 2
            List<String> ribbonContents = tc.ribbon.getContents();
            tc.addStepInfo("""
                    ...
                    - Admin icon
                    """, true, ribbonContents.containsAll(List.of("Contact icon", "Settings icon", "Notification bell icon",
                    "Icon with name shortcut", "Name of logged in user", "Admin icon")), new ComparerOptions().takeScreenShotPlatform());

            //Step 3
            List<String> tileContents = tc.tile.getAllTileNames();
            tc.addStepInfo("""
                    ...
                    """, true, tileContents.containsAll(List.of("Report an issue with an order or delivery", "Show me my Requests",
                    "Question about an order or eSuport assistance", "Question about my Account", "Request Allocation or Saturday Delivery (SET Request)")),
                    new ComparerOptions().takeScreenShotPlatform());
            ...
            WaitFor.condition(() -> tc.page.exists(EPage.EXTERNAL_SALES_TOOL));
            tc.addStepInfo("User is redirected to external Sales Efficiency tool page", true, tc.page.exists(EPage.EXTERNAL_SALES_TOOL),
                    new ComparerOptions().takeScreenShotPlatform());
        });
    }
}

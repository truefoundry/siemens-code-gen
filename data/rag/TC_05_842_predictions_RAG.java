...
public class TC05_Admin_Icon_Functionality_Check
{
    ...
    void Admin_Icon_Functionality_Check()
    {
        IocBuilder.execute(Duration.ofMinutes(10), EResultData.ADMIN, "842", tc ->
        {
            ...
            WaitFor.condition(() -> !tc.table.getItemsFromColumn(ETable.USER_TABLE, EColumn.byIndex(1)).isEmpty());
            boolean usersDisplayed = !tc.table.getItemsFromColumn(ETable.USER_TABLE, EColumn.byIndex(1)).isEmpty();
            tc.addStepInfo("Only users matching search conditions are displayed", true, usersDisplayed,
                    new ComparerOptions().takeScreenShotPlatform());

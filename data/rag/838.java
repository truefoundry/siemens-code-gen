    ...
    void Landing_Page_Content_Check()
    {
        IocBuilder.execute(Duration.ofMinutes(10), EResultData.ADMIN, "842", tc ->
        {
            ...
            tc.addStepInfo("""
                    ...
                    - Admin icon
                    """, true, tc.ribbon.exists(ERibbon.CONTACT) && tc.ribbon.exists(ERibbon.SETTINGS),
                    new ComparerOptions().takeScreenShotPlatform());

            //Step 3
            List<String> expectedTiles = List.of(
            ...
            tc.addStepInfo("Landing page tile content is according screenshot and consists of following tiles:",
                    true, actualTiles.containsAll(expectedTiles), new ComparerOptions().takeScreenShotPlatform());
            ...
            tc.addStepInfo("Page with dashboard with all requests created by the user is opened", true,
                    tc.page.exists(EPage.DASHBOARD), new ComparerOptions().takeScreenShotPlatform());

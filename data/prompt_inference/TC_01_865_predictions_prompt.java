...
public class TC05_Landing_Page_Content_Check
{
    ...
    void Landing_Page_Content_Check()
    {
        IocBuilder.execute(Duration.ofMinutes(10), EResultData.ADMIN, "842", tc ->
        {
            ...
            //Step 2
            List<String> topRibbonItems = tc.ribbon.getAllRibbonItems();
            tc.addStepInfo("""
            ...
            tc.addStepInfo("Page with dashboard with all requests created by the user is opened", true, tc.tab.exists(ETab.MY_DASHBOARD),
                    new ComparerOptions().takeScreenShotPlatform());

            //Step 6
            tc.tile.open(ETile.QUESTION_ORDER_SUPPORT);
            WaitFor.condition(() -> tc.page.exists(EPage.QUESTION_ORDER_DETAILS));
            tc.addStepInfo("Page with details for reporting an issue is opened", true, tc.page.exists(EPage.QUESTION_ORDER_DETAILS),
                    new ComparerOptions().takeScreenShotPlatform());

            //Step 7
            tc.tile.open(ETile.QUESTION_ACCOUNT);
            WaitFor.condition(() -> tc.page.exists(EPage.QUESTION_ACCOUNT_DETAILS));
            tc.addStepInfo("Page with details for reporting an issue is opened", true, tc.page.exists(EPage.QUESTION_ACCOUNT_DETAILS),
                    new ComparerOptions().takeScreenShotPlatform());

            //Step 8
            tc.tile.open(ETile.REQUEST_ALLOCATION_SET);
            WaitFor.condition(() -> tc.browser.isRedirectedToExternalPage());
            tc.addStepInfo("User is redirected to external Sales Efficiency tool page", true, tc.browser.isRedirectedToExternalPage(),
                    new ComparerOptions().takeScreenShotPlatform());
        });
    }
}
